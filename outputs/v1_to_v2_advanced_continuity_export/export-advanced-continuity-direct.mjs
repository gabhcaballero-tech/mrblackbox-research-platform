...
const outputFile = path.join(__dirname, "V1_TO_V2_ADVANCED_CONTINUITY_EXPORT.json");
const summaryCsvFile = path.join(__dirname, "V1_TO_V2_ADVANCED_CONTINUITY_SUMMARY.csv");
...
try {
  ...
  for (const row of auditRows) {
    try {
      const exportJson = await buildParticipantExport(row.NAV_FOLIO);
      participants.push(toAdvancedContinuityParticipant(exportJson, row));
    } catch (error) {
      failed.push({
  ...
  const payload = {
    ...
    summary: {
      requestedParticipants: auditRows.length,
  ...
  await fs.writeFile(summaryCsvFile, toAdvancedSummaryCsv(participants), "utf8");
  console.log(
    JSON.stringify(
      {
        output: outputFile,
        summaryCsv: summaryCsvFile,
...
function toAdvancedContinuityParticipant(v1, auditRow) {
  const cltRotation = v1.rotations?.clt ?? {};
  const hutRotation = v1.rotations?.hut ?? {};
  const recruiter = findRecruiter(v1.answers ?? []);
  const respuestas = (v1.answers ?? []).map((answer) => ({
  ...
  const evidencias = (v1.evidence ?? []).map((item) => ({
...
function findRecruiter(answers) {
  ...
  return answer ? stringifyAnswer(answer.answerValue) : null;
}

function inferEvidenceActivity(type) {
  const [prefix] = String(type ?? "").split(":");
  if (prefix === "HUT_APPLICATION_EVIDENCE" || prefix === "HUT_APPLICATION_PHOTO_ENTRY") {
    return "HUT";
  }
  if (prefix === "NAVIGO") return "NAVIGO";
  if (prefix === "SCREENING") return "SCREENING";
...
function toAdvancedSummaryCsv(participants) {
  const headers = [
    "NAV_FOLIO",
    "HUT_FOLIO",
    "Nombre",
