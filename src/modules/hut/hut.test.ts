...
describe("HUT module foundation", () => {
  ...
  it("rotates HUT paired open questions with a stable participant-specific order", () => {
    const pairQuestions = getHutQuestions().filter((question) =>
      ["HUT_P8A_GUSTO_ABIERTO", "HUT_P9A_DISGUSTO_ABIERTO"].includes(question.code)
    );

    const firstParticipantOrder = orderHutQuestionsForParticipant(pairQuestions, "p1").map((question) => question.code);
    const secondParticipantOrder = orderHutQuestionsForParticipant(pairQuestions, "p2").map((question) => question.code);

    expect(firstParticipantOrder).toEqual(["HUT_P9A_DISGUSTO_ABIERTO", "HUT_P8A_GUSTO_ABIERTO"]);
    expect(secondParticipantOrder).toEqual(["HUT_P8A_GUSTO_ABIERTO", "HUT_P9A_DISGUSTO_ABIERTO"]);
    expect(orderHutQuestionsForParticipant(pairQuestions, "p1").map((question) => question.code)).toEqual(firstParticipantOrder);
    expect(getHutQuestionPairRotationAudit({
      participantId: "p1",
      questionCode: "HUT_P8A_GUSTO_ABIERTO"
    })).toEqual({
      group: "HUT_EVA1_LIKES_DISLIKES",
      order: firstParticipantOrder
    });
  });
  ...
  it("tracks HUT v5 progress by questionnaire section", async () => {
    ...
    const participantId = participant.ok ? participant.data.participantId : "";
    const createdParticipant = prisma.state.participants.find((item) => item.id === participantId)!;
    createdParticipant.origin = "CLT_HUT";
    completeProduct1AndAuthorizeSecondStageForTest(prisma.state, createdParticipant);
  ...
  it("audits the displayed order when saving a rotated HUT question pair", async () => {
    const { prisma } = createFakeHutPrisma();
    const repository = createHutRepository(prisma as never);
    const participant = await repository.createParticipant({
      name: "Participante Par Rotado",
      requestOrigin: "https://example.com",
      studyId: "study-hut"
    });
    const participantId = participant.ok ? participant.data.participantId : "";
    ...
    const result = await repository.saveQuestionnaireAnswer({
      ...
      answerInput: { HUT_P8A_GUSTO_ABIERTO: "Me gusto el aroma." },
      fieldAccessAudit: {
    ...
    expect(result.ok).toBe(true);
    expect(prisma.state.answers.find((answer) => answer.questionCode === "HUT_P8A_GUSTO_ABIERTO")?.answerJson)
      .toBe("ME GUSTO EL AROMA.");
    expect(prisma.state.auditLogs.find((log) =>
      (log.afterJson as { questionCode?: string } | undefined)?.questionCode === "HUT_P8A_GUSTO_ABIERTO"
    )?.afterJson).toMatchObject({
      questionCode: "HUT_P8A_GUSTO_ABIERTO",
      questionPairRotation: getHutQuestionPairRotationAudit({
        participantId,
        questionCode: "HUT_P8A_GUSTO_ABIERTO"
      }),
      status: "ANSWER_SAVED"
  ...
  it("requires master slot 2 authorization before capturing the first perfume evaluation", async () => {
    ...
    const repository = createHutRepository(prisma as never);
    const participant = await createProduct2GateParticipant(repository, prisma);
    participant.questionnaireAttempt = null;
    prisma.state.questionnaireAttempts = [];

    const beforeAuthorization = await repository.saveQuestionnaireAnswer({
      answerInput: { HUT_P1A_USO_PERFUME: "1" },
      participantId: participant.id,
      questionCode: "HUT_P1A_USO_PERFUME",
      studyId: "study-hut"
    });
    const wrongCode = await repository.authorizeSecondStage({
      accessCode: "SUP26",
    ...
    const authorization = await repository.authorizeSecondStage({
      accessCode: "SUP26",
      accessType: "SUPERVISOR",
      actorUserId: null,
      code: "M3P9",
    ...
    const product2BeforeRelease = await repository.requestApplicationPhotoUpload({
      metadata: selfieMetadata(),
      now: new Date("2026-08-12T21:00:00.000Z"),
      slotId: "PRODUCT_2_DAY_1",
      storage,
    ...
    expect(prisma.state.auditLogs.find((log) => log.reason === "SECOND_STAGE_AUTHORIZED")?.afterJson).toMatchObject({
      accessCode: "SUP26",
      accessType: "SUPERVISOR",
      action: "SECOND_STAGE_AUTHORIZED"
    });
    expect(product2BeforeRelease).toMatchObject({
      message: "Esta foto HUT aun no esta disponible.",
      ok: false
    });
    expect(afterAuthorization).toMatchObject({
      data: { questionCode: "HUT_P1A_USO_PERFUME" },
      ok: true
    });
  });

  it("keeps legacy first perfume evaluation progress compatible without creating authorization events", async () => {
    const { prisma } = createFakeHutPrisma();
    const repository = createHutRepository(prisma as never);
    const participant = await createProduct2GateParticipant(repository, prisma);

    const result = await repository.saveQuestionnaireAnswer({
      answerInput: { HUT_P3A_MOSTRO_ENVASE: "1" },
      participantId: participant.id,
      questionCode: "HUT_P3A_MOSTRO_ENVASE",
      studyId: "study-hut"
    });
    const workspace = await repository.getFieldQuestionnaireWorkspace({ folio: participant.folio ?? "" });

    expect(result).toMatchObject({
      data: { questionCode: "HUT_P3A_MOSTRO_ENVASE" },
      ok: true
    });
    expect(workspace).toMatchObject({
      data: {
        secondStageAuthorized: true,
        warnings: ["LEGACY_PROGRESS_WITHOUT_EVENT"]
      },
      ok: true
    });
  ...
  it("keeps legacy second product progress compatible without creating release events", async () => {
    ...
    addApplicationPhotoEntry(prisma.state, participant, {
      capturedAt: new Date("2026-08-12T18:00:00.000Z"),
      useDayNumber: 4
    });
    ...
    expect(prisma.state.auditLogs.some((log) => log.reason === "SECOND_PRODUCT_RELEASED")).toBe(false);
  });
  ...
  it("keeps HUT phase code sync as historical no-op for new operational codes", async () => {
    const { prisma } = createFakeHutPrisma();
    const repository = createHutRepository(prisma as never);
    await repository.createParticipant({
      protocolVersion: "APPLICATION_PHOTO",
    ...
    expect(first).toMatchObject({ data: { created: 0, existing: 0 }, ok: true });
    expect(first.ok ? first.message : "").toContain("ParticipantReferenceCode");
    expect(second).toMatchObject({ data: { created: 0, existing: 0 }, ok: true });
    expect(prisma.state.phaseCodes).toHaveLength(0);
  });

  it("does not block HUT phase code sync when master reference slots are incomplete", async () => {
    const { prisma } = createFakeHutPrisma();
    const repository = createHutRepository(prisma as never);
    await repository.createParticipant({
      protocolVersion: "LEGACY_VIDEO",
    ...
    expect(result).toMatchObject({ data: { created: 0, existing: 0 }, ok: true });
    expect(prisma.state.phaseCodes).toHaveLength(0);
  });

  it("preserves inconsistent existing HUT phase code mappings as legacy audit", async () => {
    const { prisma } = createFakeHutPrisma();
    const repository = createHutRepository(prisma as never);
    await repository.createParticipant({
      protocolVersion: "LEGACY_VIDEO",
    ...
    participant.phaseCodes.push(prisma.state.phaseCodes[0]!);
    ...
    expect(result).toMatchObject({ data: { created: 0, existing: 1 }, ok: true });
    expect(prisma.state.phaseCodes).toHaveLength(1);
  });
  ...
  it("keeps phase codes auditable without blocking application photo uploads", async () => {
    ...
    const participant = prisma.state.participants[0]!;
    participant.studyParticipant = prisma.state.confirmations[0]!.studyParticipant;
    participant.origin = "CLT_HUT";
    participant.applicationPhotoEntries.push({
  ...
  it("keeps used historical HUT phase codes valid for legacy participants", async () => {
    vi.stubEnv("HUT_PHASE_CODE_SECRET", "hut-phase-secret-for-tests");
    const { prisma } = createFakeHutPrisma();
    const repository = createHutRepository(prisma as never);
    await repository.createParticipant({
      folio: "NAV-004B",
      firstFragranceLeftArm: "247",
      name: "Participante Legacy Codigo",
      protocolVersion: "APPLICATION_PHOTO",
      requestOrigin: "https://example.com",
      secondFragranceRightArm: "583",
      startDate: new Date("2026-07-01T00:00:00.000Z"),
    ...
    participant.phaseCodes.push({
      ...
      encryptedCode: "encrypted-legacy-code",
      id: "phase-legacy-used",
      participantId: participant.id,
      phase: "COLOCACION",
      slot: 1,
      status: "USED"
  ...
  it("uses master slot 3 as the final-stage code for REGRESO_2 in APPLICATION_PHOTO", async () => {
    ...
    await repository.createParticipant({
      folio: "NAV-004C",
      firstFragranceLeftArm: "247",
      name: "Participante Sin Regreso 2",
      protocolVersion: "APPLICATION_PHOTO",
      requestOrigin: "https://example.com",
      secondFragranceRightArm: "583",
      startDate: new Date("2026-07-01T00:00:00.000Z"),
  ...
  it("lets admin recover, revoke and regenerate HUT phase codes without storing plain text", async () => {
    ...
    prisma.state.confirmations.push(confirmationWithCodes("NAV-004"));
    const participant = prisma.state.participants[0]!;
    const legacyRegreso1Code: FakeHutPhaseCode = {
      codeHash: hashHutPhaseCode("M3P9", "hut-phase-secret-for-tests"),
      encryptedCode: encryptHutPhaseCode("M3P9", "hut-phase-secret-for-tests"),
      id: "phase-admin-regreso-1",
      participantId: participant.id,
      phase: "REGRESO_1",
      slot: 2,
      status: "GENERATED"
    };
    prisma.state.phaseCodes.push(legacyRegreso1Code);
    participant.phaseCodes.push(legacyRegreso1Code);

    const synced = await repository.ensureHutPhaseCodesForParticipant({
      participantId: participant.id,
      studyId: "study-hut"
    });
    ...
    expect(synced).toMatchObject({ data: { created: 0, existing: 1 }, ok: true });
  ...
  it("sends a HUT photo reminder when the next photo slot is available", async () => {
    const { prisma } = createFakeHutPrisma();
    const whatsapp = createFakeWhatsAppRepository();
    const repository = createHutRepository(prisma as never, whatsapp);
  ...
  it("does not send a HUT photo reminder when all photo slots are completed", async () => {
    const { prisma } = createFakeHutPrisma();
    const whatsapp = createFakeWhatsAppRepository();
    const repository = createHutRepository(prisma as never, whatsapp);
    await createApplicationPhotoParticipant(repository, prisma.state, { folio: "HUT-REM-002" });
    const participant = prisma.state.participants[0]!;
    for (const useDayNumber of [0, 1, 2, 3, 4, 5, 6]) {
      addApplicationPhotoEntry(prisma.state, participant, {
        capturedAt: new Date(`2026-08-${String(9 + useDayNumber).padStart(2, "0")}T16:00:00.000Z`),
        useDayNumber
      });
    }
  ...
  it("does not send a HUT photo reminder at 4 AM and waits for the afternoon window", async () => {
    ...
    const beforeFourAm = await repository.processPhotoWhatsAppReminders({
      now: new Date("2026-08-10T09:59:00.000Z"),
      requestOrigin: "https://example.test",
      studyId: "study-hut"
    });
    stubAcceptedWhatsAppMeta();
    const atFourAm = await repository.processPhotoWhatsAppReminders({
      now: new Date("2026-08-10T10:00:00.000Z"),
      requestOrigin: "https://example.test",
      studyId: "study-hut"
    });
  ...
  it("does not duplicate a HUT photo reminder for the same slot within 24 hours", async () => {
    const { prisma } = createFakeHutPrisma();
    const whatsapp = createFakeWhatsAppRepository();
    const repository = createHutRepository(prisma as never, whatsapp);
    stubAcceptedWhatsAppMeta();
    ...
    expect(first.ok ? first.data.sent : -1).toBe(1);
...
function addApplicationPhotoEntry(
  state: ReturnType<typeof createFakeHutPrisma>["prisma"]["state"],
  participant: FakeParticipant,
  input: { capturedAt: Date; useDayNumber: number }
) {
  const entry: FakeApplicationPhotoEntry = {
    capturedAt: input.capturedAt,
    capturedLocalDate: input.capturedAt.toISOString().slice(0, 10),
    capturedLocalTimezone: "America/Mexico_City",
    id: `hut-application-photo-entry-${state.nextId++}`,
    participantId: participant.id,
    privateStorageKey: `hut/${participant.id}/${input.useDayNumber}.jpg`,
    productCode: input.useDayNumber >= 4 ? "583" : input.useDayNumber === 0 ? null : "247",
...
function completeProduct1AndAuthorizeSecondStageForTest(
  state: ReturnType<typeof createFakeHutPrisma>["prisma"]["state"],
  participant: FakeParticipant
) {
  for (const [useDayNumber, capturedAt] of [
    [1, new Date("2026-08-09T16:00:00.000Z")],
    [2, new Date("2026-08-10T10:00:00.000Z")],
    [3, new Date("2026-08-11T10:00:00.000Z")]
  ] as const) {
    if (!participant.applicationPhotoEntries.some((entry) => entry.useDayNumber === useDayNumber)) {
      addApplicationPhotoEntry(state, participant, { capturedAt, useDayNumber });
    }
  }
  const authorizedAt = new Date("2026-08-12T18:00:00.000Z");
  participant.secondStageAuthorization = {
    accessCode: "SUP26",
    accessType: "SUPERVISOR",
    actorUserId: null,
    authorizedAt,
    authorizedAtMexicoCity: "12/08/2026, 12:00 hrs CDMX"
  };
  state.auditLogs.push({
    ...
    entityId: participant.id,
    entityType: "HutParticipant",
    reason: "SECOND_STAGE_AUTHORIZED"
  });
}
...
function stubAcceptedWhatsAppMeta() {
  ...
  vi.stubGlobal(
    "fetch",
    vi.fn(async () => ({
      json: async () => ({ messages: [{ id: "wamid.hut-photo-reminder", message_status: "accepted" }] }),
      ok: true,
      status: 200
    }))
  );
...
async function createProduct2GateParticipant(
  ...
  const participantId = created.ok ? created.data.participantId : "";
  const participant = prisma.state.participants.find((item) => item.id === participantId)!;
  const confirmation = confirmationWithCodes(`NAV-P2-${prisma.state.nextId}`);
  participant.origin = "CLT_HUT";
  participant.studyParticipantId = confirmation.studyParticipant.id;
  participant.studyParticipant = {
    ...confirmation.studyParticipant,
    ctlSessions: [{ completedAt: new Date("2026-08-09T14:00:00.000Z"), id: `ctl-${participantId}`, status: "COMPLETED" }],
    participantConfirmation: confirmation
  };
  participant.status = "BLOCK_1_CALL_PENDING";
  prisma.state.confirmations.push(confirmation);
  ...
  const prisma: FakePrisma = {
    ...
    auditLog: {
      async create(args: { data: FakeAuditLog }) {
        const record = {
          ...args.data,
          createdAt: args.data.createdAt ?? new Date("2026-07-01T12:00:00.000Z")
        };
      ...
      async findMany(args: {
        ...
        const results = state.auditLogs.filter((log) =>
          ...
          (!args.where.reason || log.reason === args.where.reason) &&
    ...
    hutParticipant: {
      ...
      async findMany(args: { where: { protocolVersion?: string; qaParticipantRun?: { is: null }; status?: { notIn?: string[] }; studyId?: string } }) {
        return state.participants.filter((participant) =>
          (!args.where.studyId || participant.studyId === args.where.studyId) &&
          (!args.where.protocolVersion || participant.protocolVersion === args.where.protocolVersion) &&
          (!args.where.status?.notIn?.includes(participant.status)) &&
          (!args.where.qaParticipantRun || !participant.qaParticipantRun)
        );
      },
...
type FakeParticipant = {
  ...
  studyParticipant: {
    ...
    participantConfirmation?: FakeParticipantConfirmation | null;
    participantProfile: {
      email: string | null;
      name: string;
      phone: string | null;
