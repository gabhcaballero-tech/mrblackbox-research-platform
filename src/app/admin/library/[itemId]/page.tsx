...
import { getLibraryItemForAdmin, type LibraryRevisionProjection } from "@/modules/question-library/service";
      ...
      <section className="rounded-lg border border-zinc-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 space-y-4">
          {item.revisions.map((revision) => (
            <RevisionCard itemId={item.id} key={revision.id} revision={revision} />
          ))}
        </div>
      </section>
    </AppShell>
    ...
    <article className="rounded-md border border-zinc-200 bg-zinc-50 p-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        ...
        {revision.status !== "RETIRED" ? (
          <form action={retireLibraryRevisionAction.bind(null, itemId, revision.id)}>
