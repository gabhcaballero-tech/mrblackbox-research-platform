...
export function OptionEditForm({
  action = updateScreenerOptionAction,
  option,
  optionActionTypes,
  questionId,
  readOnly,
  ...
  const initialValues = useMemo(() => createValuesFromOption(option), [option]);
  const [values, setValues] = useState<OptionFormValues>(initialValues);
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const nextResult = await action(studyId, questionId, option.value, formData);
    ...
    <div className="rounded-md border border-zinc-100 p-3">
      <form className="grid gap-3 md:grid-cols-4" onSubmit={handleSubmit}>
        ...
        <label className={labelClass}>
          ...
          />
          {result?.fieldErrors?.actionCode?.[0] ? (
            <span className="text-xs text-rose-700">{result.fieldErrors.actionCode[0]}</span>
          ) : null}
        </label>
        ...
        <label className={labelClass}>
          ...
          />
          {result?.fieldErrors?.actionReason?.[0] ? (
            <span className="text-xs text-rose-700">{result.fieldErrors.actionReason[0]}</span>
          ) : null}
        </label>
        ...
        <div className="md:col-span-4">
          <button className={secondaryButtonClass} disabled={disabled} type="submit">
            {isSubmitting ? UI_LABELS.common.saving : UI_LABELS.actions.saveOption}
          </button>
        </div>
      </form>

      <div className="mt-3 flex flex-wrap gap-2">
        <form action={moveScreenerOptionAction.bind(null, studyId, questionId, option.value, "up")}>
          <button className={secondaryButtonClass} disabled={readOnly} type="submit">
            {UI_LABELS.actions.moveOptionUp}
          </button>
        </form>
        <form action={moveScreenerOptionAction.bind(null, studyId, questionId, option.value, "down")}>
          <button className={secondaryButtonClass} disabled={readOnly} type="submit">
            {UI_LABELS.actions.moveOptionDown}
          </button>
        </form>
        <form action={deleteScreenerOptionAction.bind(null, studyId, questionId, option.value)}>
          <button className={secondaryButtonClass} disabled={readOnly} type="submit">
...
function createValuesFromOption(option: ScreenerOption): OptionFormValues {
  const action = option.actions[0];

  return {
    actionCode: getActionCode(action),
    actionReason: getActionReason(action),
    actionRequiresReview: getActionRequiresReview(action),
    actionType: action?.type ?? "NONE",
    isOther: option.isOther,
    label: option.label,
    otherTextRequired: option.otherTextRequired,
    value: option.value
  };
}
