      ...
      <div className="space-y-6">
        ...
        <section className="rounded-lg border border-zinc-200 bg-white p-5 shadow-sm">
          <div className="flex items-start justify-between gap-3">
            <div>
              <h2 className="text-lg font-semibold text-zinc-950">Aplicacion inicial registrada en CTL</h2>
              <p className="mt-2 text-sm text-zinc-600">
                Este momento se toma de la comparativa de 15 minutos realizada en CLT y es la base para calcular las evaluaciones posteriores de 3, 4.5 y 6 horas.
              </p>
              {data.applicationStartedAt ? (
                <p className="mt-3 text-sm font-semibold text-zinc-950">
                  Aplicacion registrada en CLT: {formatDate(data.applicationStartedAt, data.timeZoneIana)}
                </p>
              ) : null}
            </div>
            <StatusBadge status={data.applicationStartedAt ? "ready" : "planned"}>
              {data.applicationStartedAt ? "Registrada en CLT" : "Pendiente CTL"}
            </StatusBadge>
          </div>
          {!data.applicationStartedAt ? (
            <p className="mt-5 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-950">
              Tus evaluaciones se habilitaran cuando el equipo registre la comparativa de 15 minutos en CLT.
            </p>
          ) : null}
        </section>
        ...
        {data.timeline.length === 0 ? (
          <EmptyState
            ...
            description="La aplicacion inicial se registra desde CLT para calcular los horarios de T3, T4.5 y T6."
          />
        ) : (
